  ...
  useEffect(() => {
    let ctx = gsap.context(() => {
      gsap.to(marqueeRef.current, {
        ...
        duration: 15, // Tempo que leva para cruzar a tela (ajuste a velocidade aqui)
  ...
  return (
    ...
    <div className="bg-[#8950FF] py-4 xl:text-xl overflow-hidden flex w-full">
      ...
      <div ref={marqueeRef} className="flex w-max items-center">
        ...
        <div className="flex items-center w-max pr-18">
          {words.map((word, index) => (
            <div className="flex items-center gap-18 pl-18" key={`grupo1-${index}`}>
              <span className="text-bg-white whitespace-nowrap">{word}</span>
        ...
        <div className="flex items-center w-max pr-18">
          {words.map((word, index) => (
            <div className="flex items-center gap-18 pl-18" key={`grupo2-${index}`}>
              <span className="text-bg-white whitespace-nowrap">{word}</span>
              <Image src="icons/icon-estrela-white.svg" alt="icon estrela" width={24} height={24} />
            </div>
          ))}
